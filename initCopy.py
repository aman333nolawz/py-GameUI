...
class Button:
    ...
    def draw(self, win):
        if self.border != 0:
            ...
            if self.clicked:
                pygame.draw.rect(self.image, self.cc, (self.border, self.border, 
                                 self.width - (self.border * 2), self.height - (self.border * 2)))
            elif self.hovered:
                pygame.draw.rect(self.image, self.hc, (self.border, self.border, 
                                 self.width - (self.border * 2), self.height - (self.border * 2)))
            else:
                pygame.draw.rect(self.image, self.bg, (self.border, self.border, 
                                 self.width - (self.border * 2), self.height - (self.border * 2)))
...
class Text_box:
    def __init__(self, x=0, y=0, width=75, height=30,
        ...
        self.specials = [8, 32]
        self.alphabets = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxys'
    ...
    def draw(self, window):
        ...
        if not self.active:
            ...
            if text_width < self.width - self.border * 2:
                ...
                self.image.blit(text, ((self.border * 2) + (self.width - text_width - self.border * 3), 
    ...
    def add_text(self, key):
        try:
            ...
            if key in [303, 304]:
                self.caps = True
            elif chr(key) in self.alphabets:
                if self.caps:
    ...
    def __init__(self,  x: int,
        ...
        """A Slider GUI for pygame.
            params:
                ...
                from_: int  default=0                                #Where The value starts from
                to_: int = 3                                         #Where The value ends
                part_size: int  default = 10                         #The size for one part in the slider
                bg: tuple   default = (54, 54, 54)                   #Background color of the Slider
                cc: tuple   default = (13, 80, 213)                  #Color when the thumb is clicked
                hc: tuple   default = (160, 160, 160)                #Color when the thumb is hovered
                fg: tuple   default = (255, 255, 255)                #Color of the font to be displayed
                thumb_bg: tuple     default = (124, 124, 124)        #Background color of the thumb
                font: pygame.font   default = pygame.font.SysFont("arial", 16) #the font of the numbers
                show_numbers: bool  default = True                   #True if you want the numbers to be shown
        """
        ...
        self.thumb = pygame.Rect(0, 0, 
                                 thumb_width, self.height)
